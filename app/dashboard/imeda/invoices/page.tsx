...
export default function InvoicesPage() {
    ...
    const loadInvoices = async () => {
        setLoading(true);
        try {
            const data = await getInvoices('imeda');
            setInvoices(data);
        } catch (error) {
    ...
    return (
        <div className="space-y-6">
            <div className="flex justify-between items-center">
                ...
                <Link
                    href="/dashboard/imeda/invoices/create"
                    className="inline-flex items-center px-4 py-2 border border-transparent text-sm font-medium rounded-md shadow-sm text-white bg-imeda hover:bg-imeda/90 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-imeda transition-colors"
                >
                        ...
                        <tbody className="bg-white dark:bg-zinc-800 divide-y divide-gray-200 dark:divide-gray-700">
                            {loading ? (
                                ...
                                invoices.map((invoice) => (
                                    <tr key={invoice.id} className="hover:bg-gray-50 dark:hover:bg-zinc-700/50 transition-colors cursor-pointer" onClick={() => window.location.href = `/dashboard/imeda/invoices/${invoice.id}`}>
                                        ...
                                        <td className="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                                            <Link href={`/dashboard/imeda/invoices/${invoice.id}`} className="text-imeda hover:text-imeda/80">
                                                View
                                            </Link>
                                        </td>
                                    </tr>
