...
import type { NewInvoiceData } from '@/types/invoice';
...
export default function CreateInvoicePage() {
    ...
    const { register, control, handleSubmit, watch, setValue, formState: { errors } } = useForm<NewInvoiceData>({
        defaultValues: {
            unit_id: 'afconsult',
            ...
            date_due: new Date(Date.now() + 30 * 24 * 60 * 60 * 1000).toISOString().split('T')[0] as any,
            items: [{ id: '1', description: '', details: '', quantity: 1, rate: 0, amount: 0 }],
            vat_rate: 0,
            status: 'DRAFT',
            currency: 'AED',
            notes: ''
    ...
    useEffect(() => {
        if (selectedBank && selectedBank.currency !== selectedCurrency) {
    ...
    }, [selectedCurrency, companyProfile]);


    const loadData = async () => {
        const [clientsData, profileData, brandData] = await Promise.all([
            getClients('afconsult'),
            getCompanyProfile(),
            getBrandBySlug('afconsult')
        ]);
        ...
        if (profileData?.bank_accounts) {
            const defaultBank = profileData.bank_accounts.find(b => b.is_default && b.currency === 'AED')
                || profileData.bank_accounts.find(b => b.currency === 'AED');
        ...
        <div className="space-y-6">
            ...
            <form onSubmit={handleSubmit(onSubmit)} className="space-y-8">
                    ...
                    <div>
                                ...
                                <div className="space-y-2">
                                    <div className="flex items-center justify-end gap-4">
                                        ...
                                        <input
                                            ...
                                            className="text-right font-mono border-b border-gray-300 focus:border-afconsult outline-none w-32"
                                        />
                                    </div>
                                    <div className="flex items-center justify-end gap-4">
                                    ...
                                    <div className="flex items-center justify-end gap-4">
                                        ...
                                        <select
                                            {...register('currency')}
                                            className="text-right font-mono border-b border-gray-300 focus:border-afconsult outline-none w-32 bg-transparent"
                                        >
                        ...
                        <div className="mb-12">
                            ...
                            <select
                                ...
                                className="w-full md:w-1/2 p-2 border border-gray-300 rounded focus:ring-1 focus:ring-afconsult outline-none"
                            >
                        ...
                        <div className="mb-8">
                                            ...
                                            <td className="py-2 pr-4">
                                                <div className="space-y-1">
                                                    <input
                                                        ...
                                                        className="w-full p-1 border border-transparent hover:border-gray-200 focus:border-afconsult outline-none rounded transition-colors font-medium"
                                                    />
                                                    <input
                                                        {...register(`items.${index}.details` as const)}
                                                        placeholder="Additional details (optional)"
                                                        className="w-full p-1 text-xs text-gray-500 border border-transparent hover:border-gray-200 focus:border-afconsult outline-none rounded transition-colors"
                                                    />
                                                </div>
                                            </td>
                                            <td className="py-2 pr-4">
                                                <input
                                                    ...
                                                    className="w-full text-right p-1 border border-transparent hover:border-gray-200 focus:border-afconsult outline-none rounded"
                                                />
                                            </td>
                                            <td className="py-2 pr-4">
                                                <input
                            ...
                            <button
                                ...
                                className="flex items-center gap-2 text-afconsult hover:text-afconsult/80 font-medium text-xs px-2 py-1"
                            >
                    ...
                    <div>
                        ...
                        <div className="mb-8">
                            ...
                            <textarea
                                {...register('notes')}
                                rows={2}
                                className="w-full p-3 border border-gray-200 rounded resize-none focus:border-afconsult outline-none text-gray-600 text-sm bg-gray-50"
                            ...
                            <div className="space-y-6">
                                ...
                                <div>
                                    ...
                                    <p className="text-xs text-gray-500 leading-relaxed">
                                        Once the transfer has been completed, please send a payment confirmation or transfer receipt to the following address: <span className="text-afconsult font-medium">{contactEmail}</span>. This will allow us to track your payment and confirm your registration without delay.
                                    </p>
                                </div>
                            </div>
                ...
                <div className="fixed bottom-6 right-6 flex gap-4">
                    <button
                        type="button"
                        onClick={() => router.push('/dashboard/afconsult/invoices')}
                    ...
                    <button
                        type="submit"
                        disabled={submitting}
                        className="px-6 py-3 bg-afconsult text-white font-medium rounded-full shadow-lg hover:bg-afconsult/90 transition-all flex items-center gap-2 disabled:opacity-50 disabled:cursor-not-allowed"
                    >
                        {submitting ? (
