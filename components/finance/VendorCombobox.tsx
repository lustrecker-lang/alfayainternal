...
import { getVendors, addVendor, type Vendor } from '@/lib/finance';
...
export default function VendorCombobox({
    ...
    const showAddOption = search.trim() !== '' &&
        !vendors.some(v => v.name.toLowerCase() === search.trim().toLowerCase());
    ...
    const handleAddNew = async () => {
        ...
        setIsAdding(true);
        try {
            const newVendor = await addVendor(search.trim());
            setVendors([...vendors, newVendor].sort((a, b) => a.name.localeCompare(b.name)));
        ...
        <div ref={dropdownRef} className={`relative ${className}`}>
            ...
            <div className="relative">
                <input
                    ...
                    className="w-full px-3 py-2 pr-8 border border-gray-300 dark:border-gray-600 bg-white dark:bg-zinc-800 text-gray-900 dark:text-white font-sans text-sm focus:outline-none focus:ring-2 focus:ring-blue-500"
                ...
                >
                    {filteredVendors.length > 0 ? (
                            ...
                            {filteredVendors.map((vendor) => (
                                <li key={vendor.id}>
                                    <button
                                        ...
                                        className={`w-full px-3 py-2 text-left text-sm font-sans hover:bg-gray-100 dark:hover:bg-zinc-700 transition-colors ${vendor.name === value
                        ...
                        <div className="px-3 py-2 text-sm text-gray-500 font-sans">
                            {search ? 'No matches found' : 'No vendors yet'}
                        </div>
                    )}
                    ...
                    {showAddOption && (
                        ...
                        >
                            ...
                            {isAdding ? 'Adding...' : `Add "${search.trim()}" as new vendor`}
                        </button>
                    )}
                </div>
            )}
